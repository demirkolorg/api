        ...
        if (i >= expectedRoles.length) {
          let response = Response.errorResponse(
            new CustomError(
              ...
              i18n.translate("AUTH.UNAUTHORIZED_ACCESS_TITLE", req.user.language),
              i18n.translate("AUTH.UNAUTHORIZED_ACCESS_DESC", req.user.language)
            )
          );
          return res.status(response.code).json(response);
        }
        return next(); //Auth succcess
      };
    },
  };
};
